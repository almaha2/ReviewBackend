            ...
            <nav id="navbar" class="navbar">
                <ul>
                    ...
                    $sql = $con->prepare("SELECT e.`company_name`, e.`name` ,  u.`username` FROM `reviews` e 
                                      ...
                                      WHERE e.`category_name`='Service' OR e.`company_name`
                    ...
                    <form action="search.php" style="display: inline-flex;" method="GET">
                        <li><input type="text" name="nameCompanyUser" class="form-control"
                                   placeholder="search by company name or review name or username"
                                   style="width: 500px;background: #d3f9f9; margin-left: 3px"></a></li>
                        <li><input type="submit" style="background: #5e8078;height: 40px; margin-left: 3px"
                                   class="btn text-white" value="Search">
                        </li>
                    </form>
        ...
        <section class="container">
            <div class="row d-flex justify-content-center mt-5">
                <div class="col-md-10 col-xl-8 text-center">
                    <h3 class="mb-4">Review Of Service</h3>
                    <p class="mb-4 pb-2 mb-md-5 pb-md-0">
                        This is All Category of Service Review And All details it
                    </p>
                </div>
            </div>
...
//                    }
            ...
            <div class="row text-center">
                ...
                $sql = $con->prepare("SELECT e.*, u.`username` FROM `reviews` e 
                                    INNER JOIN users u ON e.user_id = u.id
                                    AND e.`category_name` = 'Service' ORDER BY e.`rate` desc "
                );
                $sql->execute();
                $rows = $sql->fetchAll();
                //        echo "<pre>";
