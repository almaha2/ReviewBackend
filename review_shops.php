            ...
            <nav id="navbar" class="navbar">
                <ul>
                    ...
                    $sql = $con->prepare("SELECT e.`company_name`, e.`name` ,  u.`username` FROM `reviews` e 
                                      ...
                                      WHERE e.`category_name`='Shop' OR e.`company_name`
        ...
        <section class="container">
            <div class="row d-flex justify-content-center mt-5">
                <div class="col-md-10 col-xl-8 text-center">
                    <h3 class="mb-4">Review Of Shops</h3>
                    <p class="mb-4 pb-2 mb-md-5 pb-md-0">
                        This is All Category of Shops Review And All details it
                    </p>
                </div>
            </div>
...
//                    }
            ...
            <div class="row text-center">
                ...
                $sql = $con->prepare("SELECT e.*, u.`username` FROM `reviews` e 
                                    INNER JOIN users u ON e.user_id = u.id
                                    AND e.`category_name` = 'Shop' ORDER BY e.`rate` desc "
                );
                $sql->execute();
                $rows = $sql->fetchAll();
                $count = $sql->rowCount();
